...
- Look for markers like `go.mod` file or `build/` directory
...
- Create `findBeatRoot()` helper function
  - Use `os.Getwd()` to get current working directory
  - Walk up directory tree looking for `go.mod` or `build/` directory
...
- `libbeat/tests/integration/framework.go`
  ...
  - Add `findBeatRoot()` helper function to locate Beat root directory
...
3. **Beat Root Detection**: How should we find the Beat root?
   - Walk up from current working directory (via `os.Getwd()`) looking for `go.mod`
   - Walk up looking for `build/` directory
   - **Decision needed**: Prefer `go.mod` or `build/` directory as marker?
...
func findBeatRoot() string {
    ...
    // Walk up the directory tree looking for go.mod or build/ directory
    for {
        // Check for go.mod (indicates Go module root, likely Beat root)
        goModPath := filepath.Join(dir, "go.mod")
        if _, err := os.Stat(goModPath); err == nil {
            return dir
        }
        
        // Check for build/ directory (indicates Beat root)
